...
template <typename T> class is_ostreamable
{
    ...
    static decltype(operator<<(std::declval<std::ostream &>(), std::declval<const T1>())) test(
        std::ostream &s, const T1 &t);
#else
...
template <typename... arglist> struct ActualInvocation : public Invocation
{
    ...
    ActualInvocation(
        unsigned int ordinal, MethodInfo &method, const typename fakeit::production_arg<arglist>::type... args)
...
struct NoMoreInvocationsVerificationEvent : public VerificationEvent
{
    ...
    NoMoreInvocationsVerificationEvent(
        std::vector<Invocation *> &allTheIvocations, std::vector<Invocation *> &anUnverifedIvocations)
...
struct SequenceVerificationEvent : public VerificationEvent
{
    ...
    SequenceVerificationEvent(
        VerificationType aVerificationType, std::vector<Sequence *> &anExpectedPattern,
...
#define BOOST_TEST_TOOL_IMPL2(func, P, check_descr, TL, CT, F, L)                                                      \
    ::boost::test_tools::tt_detail::func(                                                                              \
        P, ::boost::unit_test::lazy_ostream::instance() << check_descr, F, static_cast<std::size_t>(L),                \
        ::boost::test_tools::tt_detail::TL, ::boost::test_tools::tt_detail::CT, 0)
...
struct BoostTestAdapter : public EventHandler
{
    ...
    BoostTestAdapter(EventFormatter &formatter) : _formatter(formatter)
    {
    }
...
private:
    ...
    void boost_fail(std::string file, int line, std::string format)
    {
        ...
        ::boost::test_tools::tt_detail::BOOST_PP_IF(2, report_assertion, 2)(
            false, BOOST_TEST_LAZY_MSG(format), file, static_cast<std::size_t>(line),
            ::boost::test_tools::tt_detail::REQUIRE, ::boost::test_tools::tt_detail::CHECK_MSG, 0);
#else
        ::boost::test_tools::tt_detail::check_impl(
            false, ::boost::unit_test::lazy_ostream::instance() << format, file, static_cast<std::size_t>(line),
            ::boost::test_tools::tt_detail::CHECK, ::boost::test_tools::tt_detail::CHECK_MSG, 0);
#endif
    }
};
...
private:
    ...
    static_assert(
        sizeof(unsigned int (SimpleType::*)()) == sizeof(unsigned int (C::*)()),
...
template <typename T> class is_simple_inheritance_layout : public has_one_base<typename std::tr2::direct_bases<T>::type>
{
};
} // namespace fakeit
...
struct TupleDispatcher
{
    ...
    template <
        std::size_t I, typename TupleType, typename FunctionType,
        typename =
    ...
    template <
        std::size_t I, typename TupleType1, typename TupleType2, typename FunctionType,
...
template <typename... arglist> struct ArgumentsMatcherInvocationMatcher : public ActualInvocation<arglist...>::Matcher
{
...
public:
    ...
    template <typename Head, typename... Tail>
    typename std::enable_if<std::is_same<AnyMatcher, Head>::value, void>::type CollectMatchers(
        const Head &head, const Tail &... tail)
    {
        CollectMatchers(head);
        MatchersCollector<index + 1, arglist...> c(_matchers);
...
public:
    ...
    template <
        class DATA_TYPE, typename... arglist,
...
struct InvocationUtils
{
    ...
    static void collectActualInvocations(
        std::unordered_set<Invocation *> &actualInvocations, std::vector<ActualInvocationsSource *> &invocationSources)
    {
    ...
    static void selectNonVerifiedInvocations(
        std::unordered_set<Invocation *> &actualInvocations, std::unordered_set<Invocation *> &into)
    {
    ...
    static void collectInvocationSources(
        std::vector<ActualInvocationsSource *> &into, const ActualInvocationsSource &mock, const list &... tail)
    {
    ...
    static void collectInvolvedMocks(
        std::vector<Sequence *> &allSequences, std::vector<ActualInvocationsSource *> &involvedMocks)
    {
...
private:
    static void getActualInvocationSequence(
        InvocationsSourceProxy &involvedMocks, std::vector<Invocation *> &actualSequence)
    {
    ...
    static int countMatches(
        std::vector<Sequence *> &pattern, std::vector<Invocation *> &actualSequence,
        std::vector<Invocation *> &matchedInvocations)
    {
        int end = -1;
        int count = 0;
    ...
    static void collectActualInvocations(
        InvocationsSourceProxy &involvedMocks, std::unordered_set<Invocation *> &actualInvocations)
    {
        involvedMocks.getActualInvocations(actualInvocations);
    }

    static bool findNextMatch(
        std::vector<Sequence *> &pattern, std::vector<Invocation *> &actualSequence, int startSearchIndex, int &end,
        std::vector<Invocation *> &matchedInvocations)
    {
    ...
    static void collectMatchedInvocations(
        std::vector<Invocation *> &actualSequence, std::vector<Invocation *> &matchedInvocations, int start, int length)
    {
    ...
    static bool isMatch(
        std::vector<Invocation *> &actualSequence, std::vector<Invocation::Matcher *> &expectedSequence, int start)
    {
        bool found = true;
...
private:
    ...
    SequenceVerificationExpectation(
        VerificationEventHandler &fakeit, InvocationsSourceProxy mocks, std::vector<Sequence *> &expectedPattern)
    ...
    template <typename... list>
    std::vector<Sequence *> &collectSequences(
        std::vector<Sequence *> &vec, const Sequence &sequence, const list &... tail)
    {
        vec.push_back(&const_cast<Sequence &>(sequence));
    ...
    void handleExactVerificationEvent(
        VerificationEventHandler &verificationErrorHandler, std::vector<Invocation *> actualSequence, int count)
    {
        ...
        VerificationEventHandler &verificationErrorHandler, std::vector<Invocation *> actualSequence, int count)
    {
        SequenceVerificationEvent evt(
            VerificationType::AtLeast, _expectedPattern, actualSequence, -_expectedCount, count);
        evt.setFileInfo(_file, _line, _testMethod);
        return verificationErrorHandler.handle(evt);
    }
};
...
class SequenceVerificationProgress
{
    ...
    SequenceVerificationProgress(
        FakeitContext &fakeit, InvocationsSourceProxy sources, std::vector<Sequence *> &allSequences)
...
public:
    ...
    template <typename... list> UsingProgress operator()(const ActualInvocationsSource &head, const list &... tail)
    {
        std::vector<ActualInvocationsSource *> allMocks{
            &InvocationUtils::remove_const(head), &InvocationUtils::remove_const(tail)...};
...
public:
    ...
    template <typename... list> SequenceVerificationProgress operator()(const Sequence &sequence, const list &... tail)
    {
        std::vector<Sequence *> allSequences{
            &InvocationUtils::remove_const(sequence), &InvocationUtils::remove_const(tail)...};
...
class VerifyNoOtherInvocationsVerificationProgress
{
    ...
    private:
        ...
        VerifyNoOtherInvocationsExpectation(
            VerificationEventHandler &fakeit, std::vector<ActualInvocationsSource *> mocks)
    ...
    VerifyNoOtherInvocationsVerificationProgress(
        FakeitContext &fakeit, std::vector<ActualInvocationsSource *> &invocationSources)
        : VerifyNoOtherInvocationsVerificationProgress(
